...
class Module extends Singleton
{
    ...
    public function launch($names, $data = null, $settings = null, $caching = 'default')
    {
        ...
        // на данный момент представляется либо через foreach готовых настроек,
        // либо через сериализацию-десериализацию массива
        // либо уже оставить обработку при выводе шаблона, хотя это не достаточно автоматизирует процесс
        // чтение языковых файлов сейчас возможно через чтение языков или через view,
        // а дополнительные языковые переменные прописывать в конфиге
