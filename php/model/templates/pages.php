...
class Pages extends View {
	...
	public function setPagePath() {
		...
		$path = $this -> path . 'html' . DS . ($section ? 'section' . DS . $section . DS : 'inner' . DS);
	...
	public function setBlockPath($name) {
		...
		$this -> path_block = $parent . $array[0] . DS . 'html' . DS . 'blocks' . DS . $array[1] . '.php';
	...
	public function loadPage($name = null) {
		if (!$name) {
			...
			$path = $this -> path . 'html' . DS . 'inner' . DS . $this -> parsePagePath($name) . '.php';
			//echo '+' . $path;
		}
		$this -> loadPath($path);
	}
