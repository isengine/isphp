...
class System
{
    ...
    public static function server($name, $from = null)
    {
        ...
        if ($name === 'root') {
            ...
            $name = realpath(self::server('DOCUMENT_ROOT', true)) . DS;
        } elseif ($name === 'host') {
            // isengine.org
            ...
            $protocol = 'http';
            if (
                strtolower(substr(self::server('SERVER_PROTOCOL', true), 0, 5)) === 'https' ||
            ...
            $name = urldecode(self::server('REQUEST_URI', true));
        } elseif ($name === 'method') {
            // get
            $name = strtolower(self::server('REQUEST_METHOD', true));
        } elseif ($name === 'ip') {
            // 127.0.0.1
